...
import com.jgr.micro.yoda.zipkin.sleuth.feign.client.SuperHeroZipkinSleuthClientFeign;
...
/**
 * The Class YodaZipkinSleuthController.
 */
@RestController

//@RequestMapping("/yoda")
public class YodaZipkinSleuthController {
	...
	@Autowired
	private SuperHeroZipkinSleuthClientFeign superHeroZipkinSleuthClientFeign;
	...
	private static final Logger log = LoggerFactory.getLogger(YodaZipkinSleuthController.class);
	...
	public ResponseEntity<?> getCharactersYoda() {
		...
		return (ResponseEntity<?>) circuitBreakerFactory.create("micro-yoda-resiliencia").run(
				...
				e -> superHeroZipkinSleuthClientFeign.getCharactersFailOver()) // esto si da error
	...
	private ResponseEntity<?> alternativaError(Throwable e) {
		log.info("alternativaError" + e.getLocalizedMessage() + e.getMessage());

		return superHeroZipkinSleuthClientFeign.getCharactersFailOver();

		// return (Iterable<String>)
		// superHeroZipkinSleuthClientFeign.getCharactersAlternativa().getBody();
	}
